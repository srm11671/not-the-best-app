...
export function FanJoinForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError("")
    try {
      const res = await fetch("/api/teams/fan-join", {
        method: "POST",
  ...
  return (
    <div className="mx-auto max-w-2xl px-6 pb-16">
      ...
      <form onSubmit={handleSubmit} className="paper-card space-y-6 rounded-md p-8">
        <h2 className="font-display text-2xl font-semibold">Follow a Team as a Fan</h2>
        <p className="text-sm" style={{ color: "var(--ink-soft)" }}>
          Ask the team for their fan code to view their visits and join the conversation.
        </p>
        ...
        <div>
          <label className={labelClass}>Fan code</label>
          <input
            required
        ...
        >
          {submitting ? "Joining..." : "View This Team"}
        </button>
      </form>
    </div>
  )
