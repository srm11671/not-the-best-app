...
interface FanCodePanelProps {
  teamId: string
  fanCode: string
}

export function FanCodePanel({ teamId, fanCode }: FanCodePanelProps) {
  const [code, setCode] = useState(fanCode)
  ...
  async function handleRegenerate() {
    ...
    try {
      const res = await fetch(`/api/teams/${teamId}/regenerate-fan-code`, { method: "POST" })
      const data = await res.json()
      if (res.ok) setCode(data.fanCode)
    } finally {
  ...
  return (
    <div className="paper-card rounded-md p-4">
      <div className="text-xs stamp mb-2" style={{ color: "var(--ink-soft)" }}>
        Fan code (share for read-only viewing + comments)
      </div>
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          style={{ borderColor: "var(--line)" }}
          aria-label="Regenerate fan code"
        >
